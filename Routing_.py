...
class Routing(object):
    ...
    def __init__(self,tau,Distances,Dict_GWs,Num_GW,kind,Hparameter,NYM = False):
    ...
    def SortDis1(self,List): #We want to make a row of distance sorted acsending

  
        #matrix
        INV=np.zeros((len(List),len(List)))
        #We save the information for sorting the matrix later
        if True:
        ...
        return  np.sort(np.matrix(List), axis = 1), np.transpose(B)
    ...
    def PDF(self,List):
        ...
        Dis1,Inv = self.SortDis1(List)
    
        Dis2 = self.InitialDist(Dis1.tolist()[0])

        DistFuncs = list(np.dot( Dis2,Inv))
    ...
    def PDFs(self):
        ...
        for i in range(self.NG):
            j = i+1
            Dis.append(self.PDF(self.Dict_GW[gg+str(j)]))
        Dists = np.matrix(Dis)
...
from Latency_Wide import Latency_and_Dict
